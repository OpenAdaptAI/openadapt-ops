...
version, review, and ship.

!!! note "Authoritative schema"
    This page is conceptual. The exact, versioned schema lives in the engine
    repository (`DESIGN.md` for the shipping v0 IR, and
    [`WORKFLOW_PROGRAM_IR.md`](https://github.com/OpenAdaptAI/openadapt-flow/blob/main/docs/design/WORKFLOW_PROGRAM_IR.md)
...
- **The compiled workflow** (`workflow.json`): the ordered steps and, per step,
  its risk classification and identity-coverage metadata (`identity_armed`,
  `identity_unarmed_reason`). This makes a bundle auditable **before** it runs:
  you see which steps write and which clicks are identity armed without replaying
  anything.
- **Per-step evidence**: the redundant anchors the
  ...
  neighbours.
- **Postconditions**: assertions derived from what the demonstration changed on
  screen, checked after each action.
- **Effects** (when declared): typed system-of-record assertions
...
confirm, add, and siblings) are auto-classified `irreversible`, arming the
...
The bundle is the **trust boundary** between compiler and deterministic replayer:
everything the replayer does to a real system of record is frozen in
`workflow.json` and its template PNGs. Schema v2 hardens that boundary additively,
never rewriting existing semantics.
...
- **Integrity check on load**: the digest is recomputed on read, so a bundle
  whose `workflow.json` or assets were tampered with after sealing is refused,
  not silently run.
...
- **Clean migration**: a v1 bundle (or one with no `schema_version`) migrates to
  ...
  keeps replaying byte-for-byte; migration only stamps the version and back-fills
  a manifest if absent.
...
import-light (no OCR / cv2 / model dependencies), so it is safe on every load.
...
Because the workflow and its coverage metadata live in the bundle, audit it
statically:
...
`lint` and `certify` read this bundle metadata to report gaps and enforce a
policy. Every run writes its own report separately under `runs/`; see
[Read and audit run reports](../guides/run-reports.md).

## Emitting other formats from a bundle

Emit a bundle as an Agent Skills folder or a standalone MCP server, so other
agents can invoke the workflow:
...
compiled program. `compile` consumes the recording and produces the bundle;
deploy the bundle, not the recording.
