...
controls for a regulated, on-prem deployment, and the pilot install runbook.

!!! note "This is the opposite lane from the hosted option"
    An on-prem deployment uses no OpenAdapt cloud account, telemetry, cloud
    model, or auto-update. Traffic to the target application and to any
    configured system-of-record verifier still follows your deployment
    configuration. For the managed, non-regulated lane, see
    [the hosted option](hosted.md).
...
1. **No OpenAdapt egress by default in the software.** The deterministic path
   makes no OpenAdapt-hosted or model-service calls with no VLM URL set, and
   there is no telemetry, analytics, license check, or update ping in the run
   path. Target-application and configured verifier traffic still follows
   `deployment.yaml`; keep those endpoints on the LAN.
...
The firewall is the real control; the `internal:true` network, the systemd
`IPAddressDeny=any`, and `verify-airgap.sh` are defence-in-depth and attestation,
not a replacement for a correctly configured network boundary.
...
## Encryption at rest — how to set it up, and the honest caveat

Everything lives under `storage_root` (default `/srv/openadapt`), which you place
on a **full-disk-encrypted** volume (LUKS / BitLocker / FileVault — OpenAdapt
never holds the disk key). At-rest protection has three layers:
...
!!! danger "Honest at-rest caveat — the identifier crops"
    ...
    the per-bundle encrypted container** — they still rely on **operator
    full-disk encryption**. So: enable `OPENADAPT_BUNDLE_KEY` for the JSON and
    checkpoints, but do **not** represent the whole bundle as cryptographically
    sealed. Full-disk encryption remains the baseline control for the template
    crops. Treat every bundle as PHI.
...
In a clinic whose EMR is delivered over Citrix, the automation process runs
**on the clinic host inside
the clinic network**, reaching the application over the LAN via the RDP/pixel or
Windows backend — the same path a clinician's session already uses. The runner
needs LAN reachability to the Citrix/RDP endpoint and to the local EMR's
system-of-record API (for effect verification), and **nothing beyond the LAN**.
On pure-pixel Citrix/RDP the identity ladder falls back from structured a11y/DOM
text to the pixel/OCR tiers — read the wrong-patient guarantees and their
availability cost on that substrate before relying on it.
...
and none of the model tiers exist (the default install pulls no model).
...
a BAA, and no part of the software is a certification. What the software
provides — PHI processed locally, protected at rest by full-disk encryption plus
in-bundle identity hashing plus opt-in AES-256-GCM sealing, and a local
append-only audit trail — is the technical substrate those agreements attest to.
The full boundary list is in the engine repo's `deploy/on-prem/COMPLIANCE.md`.
...
caveats, and the OCR ceilings all still apply. On-prem changes *where* the data
lives, not *what the replay can and cannot guarantee*.
