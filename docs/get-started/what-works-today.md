...
and is reported distinctly from evidence, using the same canonical public
labels published in the machine-readable
[status manifest](https://openadapt.ai/status.json) that the website, launcher,
and packages also reconcile to. A label never claims more than its evidence:

- **Beta** — a supported reference path, still subject to per-workflow
  qualification.
- **Scoped acceptance — design partners only** — a real, counted scoped
  qualification passed for an exact task and environment, but no external
  customer has run it in production, so access is design-partner only.
- **Research / design-partner qualification** — no validated integration yet;
  qualification can only begin inside a design partner's real environment.
- **Beta / public offer** — a live, publicly purchasable Beta subscription;
  the maturity is Beta, not a certification, availability SLA, or completed
