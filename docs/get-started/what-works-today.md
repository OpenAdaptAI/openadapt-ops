...
| Hosted browser recorder | **Beta / bounded launch component** | A real Modal session produced PNG frames and retained input evidence, assembled a compileable recording, finalized one workflow idempotently, enforced resource limits, and cleaned up ephemeral qualification data. The qualified worker used `openadapt-flow` 1.7.3. | Explicitly initiated, public-HTTPS, non-regulated authoring only. Raw observations remain private inside the declared hosted boundary; this does not qualify paid checkout or managed replay. |
